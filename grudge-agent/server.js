...
app.get("/api/ide/tree", (req, res) => {
  const requested = req.query.dir
    ? req.query.dir
    : (req.query.project ? path.join(PROJECTS_DIR, String(req.query.project)) : PROJECTS_DIR);
  ...
  function walk(d, depth=0) {
    ...
    return fs.readdirSync(d, { withFileTypes:true })
      ...
      .map(e => {
        ...
        return { name: e.name, path: full, type: isDir ? "dir" : "file", isDir,
...
app.post("/api/ide/file", (req, res) => {
  const p = req.body.p || req.body.path;
  const content = req.body.content;
